...
// Service Client for bridge_px4/TrajTransferRequest
...
BRIDGE_PX4_EXPORT void copy_from_arr(bridge_px4::TrajTransfer::Request& request,
...
BRIDGE_PX4_EXPORT MDArray_T get_arr(MDFactory_T& factory,
                                         const bridge_px4::TrajTransfer::Response& response);
} // namespace TrajTransfer_Response
} // namespace matlabhelper
} // namespace bridge_px4
class BRIDGE_PX4_EXPORT bridge_px4_srv_TrajTransfer_client
    ...
    std::map<int64_t, bridge_px4::TrajTransfer::Request> mRequestPool;
    bridge_px4::TrajTransfer::Request mNextRequest;
  public:
    bridge_px4_srv_TrajTransfer_client()
        : MATLABSvcClientInterface()
        , mNextRequest() {
    }
    virtual ~bridge_px4_srv_TrajTransfer_client() {
    }
    virtual intptr_t createSvcClient(const std::string& srv_name,
        ...
        mClient = std::make_shared<ros::ServiceClient>(
            theNode->serviceClient<bridge_px4::TrajTransfer>(srv_name));
    ...
    virtual bool send_request(void* sd,
        ...
        if (ret) {
            ...
            auto outArray =
                bridge_px4::matlabhelper::TrajTransfer_Response::get_arr(mFactory, svc.response);
            if (sd != NULL) {
    ...
    virtual bool addRequestToPool(matlab::data::Array arr) {
        bridge_px4::TrajTransfer::Request req;
        bridge_px4::matlabhelper::TrajTransfer_Request::copy_from_arr(req, arr);
        mRequestPool.insert(
                {bridge_px4::matlabhelper::TrajTransfer_Request::get_requestId_from_arr(arr), 
                         req});
        return true;
    }
...
CLASS_LOADER_REGISTER_CLASS(bridge_px4_srv_TrajTransfer_client, MATLABSvcClientInterface)
#ifdef _MSC_VER
#pragma warning(pop)
#else
