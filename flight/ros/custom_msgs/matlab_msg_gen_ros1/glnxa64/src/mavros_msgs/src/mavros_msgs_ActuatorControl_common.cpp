...
namespace ActuatorControl {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const mavros_msgs::ActuatorControl::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<mavros_msgs::ActuatorControl>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //group_mix
        const matlab::data::TypedArray<uint8_t> group_mix_arr = arr[0]["group_mix"];
        msg->group_mix = group_mix_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'group_mix' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'group_mix' is wrong type; expected a uint8.");
    }
    try {
        //controls
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::ActuatorControl>& msg) {
    auto outArray = factory.createStructArray({1,1},{"PX4_MIX_FLIGHT_CONTROL","PX4_MIX_FLIGHT_CONTROL_VTOL_ALT","PX4_MIX_PAYLOAD","PX4_MIX_MANUAL_PASSTHROUGH","header","group_mix","controls"});
    // PX4_MIX_FLIGHT_CONTROL
    outArray[0]["PX4_MIX_FLIGHT_CONTROL"] = factory.createScalar(static_cast<uint8_t>(msg->PX4_MIX_FLIGHT_CONTROL));
    // PX4_MIX_FLIGHT_CONTROL_VTOL_ALT
    outArray[0]["PX4_MIX_FLIGHT_CONTROL_VTOL_ALT"] = factory.createScalar(static_cast<uint8_t>(msg->PX4_MIX_FLIGHT_CONTROL_VTOL_ALT));
    // PX4_MIX_PAYLOAD
    outArray[0]["PX4_MIX_PAYLOAD"] = factory.createScalar(static_cast<uint8_t>(msg->PX4_MIX_PAYLOAD));
    // PX4_MIX_MANUAL_PASSTHROUGH
    outArray[0]["PX4_MIX_MANUAL_PASSTHROUGH"] = factory.createScalar(static_cast<uint8_t>(msg->PX4_MIX_MANUAL_PASSTHROUGH));
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // group_mix
    outArray[0]["group_mix"] = factory.createScalar(msg->group_mix);
    // controls
    outArray[0]["controls"] = factory.createArray<mavros_msgs::ActuatorControl::_controls_type::const_iterator, float>({1, msg->controls.size()}, msg->controls.begin(), msg->controls.end());
