...
namespace TrajTransferRequest {
  //----------------------------------------------------------------------------
  BRIDGE_PX4_EXPORT void copy_from_arr(boost::shared_ptr<bridge_px4::TrajTransferRequest>& msg, const matlab::data::StructArray arr) {
    try {
        //hz
  ...
  //----------------------------------------------------------------------------
  BRIDGE_PX4_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const bridge_px4::TrajTransferRequest>& msg) {
    ...
    // u_arr
    outArray[0]["u_arr"] = factory.createArray<bridge_px4::TrajTransferRequest::_u_arr_type::const_iterator, float>({1, msg->u_arr.size()}, msg->u_arr.begin(), msg->u_arr.end());
    // x_arr
    outArray[0]["x_arr"] = factory.createArray<bridge_px4::TrajTransferRequest::_x_arr_type::const_iterator, float>({1, msg->x_arr.size()}, msg->x_arr.begin(), msg->x_arr.end());
    // L_arr
    outArray[0]["L_arr"] = factory.createArray<bridge_px4::TrajTransferRequest::_L_arr_type::const_iterator, float>({1, msg->L_arr.size()}, msg->L_arr.begin(), msg->L_arr.end());
