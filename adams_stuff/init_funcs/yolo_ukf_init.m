...
%     rel_state = get_relative_state(flight.x_act(:, 1), get_camera_state_vec(camera));
    ...
    yukf.mu = [flight.x_act(1:6, 1); flight.x_act(10:12, 1); complete_unit_quat(flight.x_act(7:9, 1))];
    yukf.mu_prev = yukf.mu;
    yukf.sigma = diag([0.1, 0.1, 0.1, 0.02, 0.02, 0.02, 0.01, 0.01, 0.01, 0.005, 0.005, 0.005])/5;
    ...
    yukf.prms.lambda = yukf.prms.alpha^2*(dim_cov + yukf.prms.kappa) - dim_cov;
    ...
    yukf.w0_m = yukf.prms.lambda / (yukf.prms.lambda + dim_cov);
    yukf.w0_c = yukf.w0_m + (1 - yukf.prms.alpha^2 + yukf.prms.beta);
    yukf.wi = 1 / (2*(yukf.prms.lambda + dim_cov));

    % yukf save variable for later plotting
    sv.mu_hist = zeros(dim_state, length(flight.t_act)); sv.mu_hist(:, 1) = yukf.mu;
    sv.mu_act = zeros(dim_state, length(flight.t_act)); sv.mu_act(:, 1) = [flight.x_act(1:6, 1); flight.x_act(10:12, 1); complete_unit_quat(flight.x_act(7:9, 1))];
    sv.sig_hist = zeros(dim_cov, dim_cov, length(flight.t_act)); sv.sig_hist(:, :, 1) = yukf.sigma;
